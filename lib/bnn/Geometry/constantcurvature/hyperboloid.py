...
from .utils.math_stereographic import mobius_add, radius_to_stereo, stereo_to_radius
...
class Hyperboloid(Hyperbolic):
    ...
    def _gyroadd_via_iso(self, x, y):
        ...
        return stereo_to_radius(res_p, self.K)
    ...
    def busemann(self, v_spatial, x):
        """
        Busemann function on the Lorentz model (vectorized):
            B^v(x) = (1 / √(-K)) · log( -√(-K) · ⟨ x , √(-K)·x⁰ + v ⟩_𝓛 ),

        where √(-K)·x⁰ = (1, 0, ..., 0), and v is a unit vector in the tangent space at the origin.

        Args:
            v_spatial: (..., n) spatial unit vector (no time component)
            x: (..., n+1) hyperboloid point(s)

        Returns:
            (...,) Busemann value(s)
        """
        return math.busemann(v_spatial, x, self.K)
