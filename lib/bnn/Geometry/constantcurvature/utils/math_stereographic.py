...
from .utils_stereographic import list_range, drop_dims, sign, clamp_abs, sabs

from .utils_ccs import EPS
...
def _inner(
    ...
    return _lambda_x(x, k, keepdim=keepdim, dim=dim) ** 2 * (u * v).sum(
...
def _mobius_matvec(m: torch.Tensor, x: torch.Tensor, k: torch.Tensor, dim: int = -1):
    ...
    # this could be wrong, which is in geoopt:
    # cond = (mx == 0).prod(dim=dim, keepdim=True, dtype=torch.bool)
    cond = (mx == 0).all(dim=dim, keepdim=True)
...
@torch.jit.script
def _radius_to_stereo(x: torch.Tensor, K: torch.Tensor, eps: float) -> torch.Tensor:
    xi = x[..., :1]               # time-like component ξ
    spatial = x[..., 1:]          # spatial part
    denom = (1.0 + K.abs().sqrt() * xi).clamp_min(eps)
    return spatial / denom

def radius_to_stereo(x: torch.Tensor, K: torch.Tensor) -> torch.Tensor:
    return _radius_to_stereo(x, K, EPS[x.dtype])

@torch.jit.script
def _stereo_to_radius(y: torch.Tensor, K: torch.Tensor, eps: float) -> torch.Tensor:
    norm_sq = (y * y).sum(dim=-1, keepdim=True)
    denom = (1.0 + K * norm_sq).clamp_min(eps)
    time = (1.0 - K * norm_sq) / (denom * K.abs().sqrt())
    space = 2.0 * y / denom
    return torch.cat([time, space], dim=-1)

def stereo_to_radius(y: torch.Tensor, K: torch.Tensor) -> torch.Tensor:
    return _stereo_to_radius(y, K, EPS[y.dtype])

@torch.jit.script
def _busemann(v: torch.Tensor,
              x: torch.Tensor,
              K: torch.Tensor,
              eps: float) -> torch.Tensor:
    sqrt_c = torch.sqrt((-K).clamp_min(eps))
    diff = v - sqrt_c * x
    numerator = (diff * diff).sum(dim=-1)
    norm_sq = (x * x).sum(dim=-1)
    denominator = (1.0 + K * norm_sq).clamp_min(eps)
    return torch.log(numerator / denominator) / sqrt_c

def busemann(v: torch.Tensor, x: torch.Tensor, K: torch.Tensor) -> torch.Tensor:
    return _busemann(v, x, K, EPS[K.dtype])
