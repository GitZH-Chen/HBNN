import torch
from .utils_ccs import EPS, sinhdiv, divsinh
...
@torch.jit.script
def _busemann(v_spatial: torch.Tensor,
                x: torch.Tensor,
                K: torch.Tensor) -> torch.Tensor:
    sqrt_c = torch.sqrt(-K)
    ones = torch.ones_like(v_spatial[..., :1])
    svec = torch.cat([ones, v_spatial], dim=-1)  # (..., n+1)
    inner = _ldot(x,svec)
    arg = (-sqrt_c) * inner
    return torch.log(arg) / sqrt_c
...
def busemann(v_spatial, x, K):
    return _busemann(v_spatial, x, K)