...
from lib.bnn.Auxlayers import LorentzBFCWrapper
...
class LorentzConv1d(nn.Module):
    ...
    def __init__(
        ...
        if conv_type == "LFC":
            ...
            self.linearized_kernel = LorentzBFCWrapper(
                manifold=self.manifold,
                in_dim=lin_features - 1,
                out_dim=self.out_channels - 1,
                bias=bias,
                gyrobias=gyrobias,
            )
        else:
            raise ValueError(f"Unsupported linear_type: {conv_type}")
